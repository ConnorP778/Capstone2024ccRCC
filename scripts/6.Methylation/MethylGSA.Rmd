...
```{r}
fwrite(merged_df, paste0(base_dir, "Output/meth_bmi.csv"))
```

```{r}
merged_df = fread(paste0(base_dir, "Output/meth_bmi.csv"), sep=",")
...
```{r}
#IMPORTANT: This could take ~2 hours. If you just want the output file, we have put it in the Output folder. Just read section in lines 171-174
...
fwrite(results, paste0(base_dir, "Output/cpg_diff_adj.csv"))
```

```{r}
#Load results from analysis
results = fread(paste0(base_dir, "Output/cpg_diff_adj.csv"), sep = ",")
```
...
```
```{r}
write.csv(hallmark, paste0(base_dir, "Output/Hallmark_methylGSA_result.csv"))
write.csv(cancer, paste0(base_dir, "Output/Cancer_methylGSA_result.csv"))
write.csv(kegg, paste0(base_dir, "Output/Kegg_methylGSA_result.csv"))
```
...
ggsave(paste0(base_dir, "Output/Plots/plot1_methylation.png"), plot1, width = 10, height = 6)
...
ggsave(paste0(base_dir, "Output/Plots/plot2_methylation.png"), plot2, width = 10, height = 6)
