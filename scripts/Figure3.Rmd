...
```
```{r}
save(result, file = "iClusterPlus_Result")
...
count_data <- count_data %>% mutate(bmi = factor(bmi)) %>% mutate(bmi = fct_recode(bmi, Normal = "normal", Overweight = "overweight",Obese= "obese"))

# Plot the data
p = ggplot(count_data, aes(x = bmi, y = Percentage, fill = factor(Cluster))) +
